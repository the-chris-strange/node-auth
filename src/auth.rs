...
/// 1. `explicit_token` (trimmed string if non-empty).
...
/// * `verbose` - If true, enables debug logging during token resolution.
///
/// # Errors
///
/// Returns [`AuthError::Authentication`] if all credential resolution methods fail.
///
/// # Example
///
/// ```no_run
...
///
/// let token = get_credentials(Some("custom-token"), false).await?;
/// assert_eq!(token, "custom-token");
/// # Ok(())
/// # }
/// ```
pub async fn get_credentials(explicit_token: Option<&str>, verbose: bool) -> Result<String, AuthError> {
    if verbose {
    ...
    if let Some(token) = explicit_token {
        ...
        if !trimmed.is_empty() {
            log::debug!("Using explicitly provided auth token.");
...
async fn get_adc_credentials() -> Result<String, AuthError> {
    let provider = gcp_auth::provider()
        .await
        .map_err(|e| AuthError::Authentication(format!("ADC provider initialization failed: {e}")))?;
    ...
    if token_str.is_empty() {
        return Err(AuthError::Authentication("ADC returned an empty token".to_string()));
    }
...
fn get_gcloud_credentials() -> Result<String, AuthError> {
    ...
    for bin in binaries {
        ...
        match Command::new(bin).args(["auth", "print-access-token"]).output() {
...
mod tests {
    ...
    async fn test_explicit_token() {
        let token = get_credentials(Some("my-explicit-token"), false).await.unwrap();
        assert_eq!(token, "my-explicit-token");
    }

    #[tokio::test]
    async fn test_explicit_token_trims() {
        let token = get_credentials(Some("  my-token-with-spaces  \n"), false).await.unwrap();
        assert_eq!(token, "my-token-with-spaces");
    }
}